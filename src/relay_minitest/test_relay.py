...
class TestMultiRelay:
    ...
    def test_delivery_port_blocked(
        ...
        # Postfix logs "status=deferred"; madmail logs one of:
    ...
    def test_one_on_one_smtp_only(self, cmfactory, cmfactory2, relayadmin2, lp):
        ac1 = cmfactory.get_online_account()
        ac2 = cmfactory2.get_online_account()
        chat = cmfactory.get_accepted_chat(ac1, ac2)
...
def test_hide_senders_ip_address(cmfactory, ssl_context):
    ...
    for raw in raw_messages:
        assert public_ip not in email.message_from_bytes(raw).as_string()
